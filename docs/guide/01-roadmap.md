...
**运行时控制已经接到 TARS I²C。** Mac → TARS CDC → I2C2（PB10/PB11）→ 电机板 `0x50`。起停、转速、力矩、方向、母线、故障、ALERT（PA15 → TARS PD5）都走总线。UART 只出日志。空闲也能读母线。换向是速度环过零，不是停桥。TEMP / VDDA 运行中大约半秒更新一次。HardFault / Error / CSS NMI 先关 MOE；IWDG 约 2 s；运行中约 3 s 没有 I²C 会停机。

在这台电机、这套半桥、刻度 4 上，**再抠环抬不了转速上限**。12 V 上 PWM 打满；大约 150 电周期/秒以上低边窗口没了。15 V 立刻更快，卡的是电压。

扰动 3→4 已记两份（`models/captured/disturb-20260915-165447.csv` 过线）。温升 20 min 已跑完（刻度 4，`thermal-20260915-170129.csv`）：wrap 均值 66.2，无故障。`pcb/` 另开，不要和固件捆在一起。
...
I²C 遥测到 `0xC3`：母线、电荷泵、板 NTC、片内结温、时钟、保护脚、vd/vq。J15 壳温比较器能报过温，模拟量这版板没有 ADC 脚，寄存器占着写 `0x8000`。

下一步若还碰这套台架：跑 `mot_i2c.sh disturb` / `thermal`（转着时脚本自己打心跳），或把 `pcb/` 另开任务进 Git。不要再为了 12 V 转速通电。
